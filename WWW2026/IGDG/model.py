...
from IGDG.pytorch_net.net import reparameterize, Mixture_Gaussian_reparam
from IGDG.pytorch_net.util import (
    sample,
    to_cpu_recur,
    to_np_array,
    to_Variable,
...
from IGDG.utils.spectral import cal_mutual_Information
...
class LinkPredictor(torch.nn.Module): # 改这个
    ...
    def forward(self, z, e):
        ...
        x = self.lins[-1](x)
        return torch.sigmoid(x).squeeze()  # 概率值
...
class IGDGConv(MessagePassing):
    def __init__(
        ...
        **kwargs
        ...
        super(IGDGConv, self).__init__(aggr="add", **kwargs)
        ...
        self.lin = nn.Linear(heads * self.out_neurons,out_channels)
    ...
    def forward(self, x_all, edge_index_all, size=None):# 图序列
        times = len(x_all)
        dev = x_all[0].device
        n = len(x_all[0])
        ...
        self.temproal_loss_list = [[] for _ in range(times)]
        ...
        for t in range(times):
            ...
            ) = self.forward_single_graphshot(x_all[t], edge_index_all[t])# 每个快照返回输出特征、互信息、结构 KL 损失
            out = self.lin(out)
            self.temproal_loss_list[t] = cal_mutual_Information(edge_index=edge_index_all[t], num_nodes=n , num_splits=self.num_splits).to(dev)# 时间损失
            if t > 0:
                weights = F.sigmoid(
                ...
                out_list[t] = out
        ixz_mean = torch.mean(torch.stack(self.ixz_list))# 平均互信息、结构 KL 损失、时间损失
        structure_kl_loss_mean = torch.mean(torch.stack(self.structure_kl_loss_list))
        temproal_loss_mean = torch.mean(torch.stack(self.temproal_loss_list))
        return out_list, ixz_mean, structure_kl_loss_mean, temproal_loss_mean * self.alpha.sum(dim=-1)
...
class IGDGNN(torch.nn.Module):
    def __init__(self, args=None):

        super(IGDGNN, self).__init__()
