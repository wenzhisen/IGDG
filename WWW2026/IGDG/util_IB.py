...
sys.path.append(os.path.join(os.path.dirname("__file__"), "..", ".."))
from IGDG.pytorch_net.util import to_np_array
from IGDG.DeepRobust.deeprobust.graph.defense import GCN
...
def get_root_dir():
    ...
    index = dirname_split.index("WWW2026")
    dirname = "/".join(dirname_split[: index + 1])
    return dirname


IGDG_PATH = get_root_dir() + "/results"
...
def parse_filename(filename, is_adversarial=False, **kwargs):
    ...
    Args:
        ...
        is_adversarial: whether the file is from experiments/IGDG_node_adversarial_attack.
    """
    if is_adversarial:
