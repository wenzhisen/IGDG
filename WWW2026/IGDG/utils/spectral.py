...
def cal_mutual_Information(edge_index, num_nodes, num_splits):
    ...
    for index, subgraph in enumerate(subgraphs):
        H_list[index] = ( spectral_entropy( subgraph, subgraph.size(1) ))
    H_total = spectral_entropy(edge_index, num_nodes)
...
def remap_edge_index(edge_index,num_nodes):
    ...
    # 应用映射到源节点和目标节点
    remapped_src = np.array([node_mapping[node] for node in edge_index[0]])
    remapped_dst = np.array([node_mapping[node] for node in edge_index[1]])

    # 组合成新的边索引
    remapped_edge_index = np.vstack([remapped_src, remapped_dst])
    new_num_nodes = len(unique_nodes)
    return remapped_edge_index,new_num_nodes

def spectral_entropy(edge_index, num_nodes, beta=1.0):
    """
    计算复杂网络的谱熵（von Neumann entropy）

    参数:
        edge_index : torch.Tensor 或 np.ndarray, 形状为 (2, num_edges)
            边的源节点和目标节点索引（支持PyTorch CUDA张量）
        num_nodes : int
            图中实际节点数量
        beta : float, 可选
            尺度参数（默认=1.0）
    ...
    if hasattr(edge_index, 'is_cuda') and edge_index.is_cuda:
        edge_index = edge_index.cpu().numpy()
    elif hasattr(edge_index, 'device') and edge_index.device.type == 'cuda':
        edge_index = edge_index.cpu().numpy()
    else:
        edge_index = np.asarray(edge_index)

    edge_index, num_nodes = remap_edge_index(edge_index, num_nodes)

    src, dst = edge_index
    if(len(src)==0 or len(dst)==0):
        return 0
    # print(max(src), max(dst), num_nodes)
    # 1. 构建邻接矩阵 A（稀疏格式）
    data = np.ones(len(src))
    A = csr_matrix((data, (src, dst)), shape=(num_nodes, num_nodes))

    # 确保无向图对称（若输入为有向边，则转为无向）
    A = A + A.T
    A.data = np.clip(A.data, 0, 1)  # 消除重复边

    # 2. 计算度矩阵 D
    degrees = np.array(A.sum(axis=1)).flatten()
    D = csr_matrix((degrees, (np.arange(num_nodes), np.arange(num_nodes))))

    # 3. 计算拉普拉斯矩阵 L = D - A
    L = D - A

    # 4. 计算拉普拉斯矩阵的特征值（使用稀疏矩阵求解前N-1个最小特征值）
    #    注意：拉普拉斯矩阵有一个0特征值（对应连通分量），需排除
    eigenvalues = eigsh(L, k=min(num_nodes - 1, num_nodes - 1), return_eigenvectors=False, which='SM')

    # 5. 计算配分函数 Z
    exp_vals = np.exp(-beta * eigenvalues)
    Z = np.sum(exp_vals)  # 公式 (4)

    # 6. 计算密度矩阵的特征值 λ_ρ
    lambda_rho = exp_vals / Z  # 公式 (6)

    # 7. 计算熵（避免 log(0)）
    entropy = 0.0
    for val in lambda_rho:
        if val > 1e-10:  # 忽略极小值避免数值不稳定
...
if __name__ == '__main__':
    ...
    for index, subgraph in enumerate(subgraphs):
        H_list[index] = ( spectral_entropy( subgraph, subgraph.size(1) ))
    H_total = spectral_entropy(graph, x.size(0))
    I_struct = H_list.sum(dim=-1) - H_total
